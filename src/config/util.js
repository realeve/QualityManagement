...
function handleAttach(html) {
  ...
  function handleFileFormat(str) {
    // rar
    str = str.replace(
      'href="data:;base64,UmFyI',
      ' download="附件.rar" href="data:application/x-rar-compressed;base64,UmFyI'
    );

    // txt
    str = str.replace(
      'href="data:text',
      ' download="附件.txt" href="data:text'
    );
  ...
  arrAttach.map((item) => {
    if (item.includes('src="data:')) {
      if (item.includes('src="data:audio/')) {
        linkStr = item.replace(
          '<img src=',
          '<audio controls="controls" autoplay="true" src='
        );
        html = html.replace(item, linkStr);
      } else if (!item.includes('src="data:image')) {
        linkStr = item
          .replace('<img src="', '<a target="_blank" href="')
          .replace('>', '><strong class="attach"> 附件' + i + ' </a></strong>');
