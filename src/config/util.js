...
function getAllReceiver (needPushPaperList = true) {
  ...
  let physic = settings.rtxInfo.physic.map(item => item.id)
  let monitor = settings.rtxInfo.monitor.map(item => item.id)

  let users = [...tech, ...imgVs, ...physic, ...monitor]
